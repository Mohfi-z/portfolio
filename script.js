...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function checkImagesLoaded() {
      imagesLoaded++;
      let percentage = Math.floor((imagesLoaded / images.length) * 100);
  ...
  function finishLoading() {
      ...
      let remainingTime = Math.max(1500 - elapsedTime, 0);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /* ========= FONCTIONS DE VÉRIFICATION (PRÉ-ET POST-CONDITIONS) ========= */
  ...
  /* ========= FONCTIONS DE DÉTECTION AVEC VÉRIFICATIONS ========= */
  function checkVirusCollision(virus) {
    ...
    const distance = Math.sqrt(dx * dx + dy * dy);
    const collision = distance < rect.width / 2;
  ...
  function updateInfiniteVirusMovement() {
    viruses.forEach((virus, index) => {
      let newLeft = virus.offsetLeft + virus.baseVelocity.x * infiniteDifficulty * difficultyMultiplier;
      let newTop = virus.offsetTop + virus.baseVelocity.y * infiniteDifficulty * difficultyMultiplier;
      virus.style.left = `${newLeft}px`;
      virus.style.top = `${newTop}px`;

      if (checkVirusCollision(virus)) {
        if (!virus.collisionStart) {
          virus.collisionStart = Date.now(); // Démarrage du timer de grâce
        } else if (Date.now() - virus.collisionStart > 100) {
          // Vérification finale avant de finir la partie
          if (checkVirusCollision(virus)) {
            endGame(false);
          }
        }
      ...
      // Suppression du virus s'il sort complètement de l'écran
      if (
  ...
  function checkInfiniteCoinCollision() {
    ...
    coins.forEach((coin, index) => {
      ...
      // Condition de délai pour une pièce "normal"
      if (coin.dataset.type === "normal" && now - coin.spawnTime >= 10000) {
        // Si le joueur est dans la zone élargie, auto-collecte la pièce
  ...
  function handleCoinCollection(coin) {
    ...
    switch (type) {
      case "slow":
        // Ralentir les virus pendant 5 sec
        difficultyMultiplier = 0.5;
        setTimeout(() => { difficultyMultiplier = 1; }, 5000);
        break;
      case "clear":
        // Supprimer 30 % des virus
      ...
      case "normal":
      default:
        // Aucune action pour la pièce normale
        break;
    }
  }
